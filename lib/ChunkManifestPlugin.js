  ...
  compiler.plugin("this-compilation", function(compilation) {
    ...
    mainTemplate.plugin("require-ensure", function(_, chunk, hash) {
      ...
      if (filename) {
        chunkManifest = [chunk].reduce(function registerChunk(manifest, c) {
          ...
          if(hasRuntime) {
            manifest[c.id] = undefined;
              ...
              manifest[c.id] = `{% static '${mainTemplate.applyPluginsWaterfall('asset-path', filename, {
                hash: hash,
                chunk: c,
              })}' %}`;
          }
        ...
        compilation.assets[manifestFilename] = new RawSource(`{% load static from staticfiles %}${JSON.stringify(chunkManifest)}`);
