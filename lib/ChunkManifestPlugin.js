          ...
          if (filename) {
            const registerChunk = (manifest, chunk) => {
              ...
              if (chunk.hasRuntime()) {
                ...
                manifest[chunk.id] = `{% static '${mainTemplate.getAssetPath(filename, {
                  hash,
                  chunk,
                })}' %}`;
              }
            ...
            // need to add this tag at the beginning of the manifest file
            // const staticTag = new RawSource('{% load static from staticfiles %}');
